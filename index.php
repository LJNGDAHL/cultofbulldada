<?php @include 'header.php'; ?>
<body class="front">
    <a class="navigation__link link" href="">Menu</a>
    <div class="fade-in fade-in__symbols">
      <img class="symbol symbol--top-left" src="assets/img/cob-symbol.svg" alt="Cult of Bulldada symbol">
      <img class="symbol symbol--top-right"src="assets/img/cob-symbol.svg" alt="Cult of Bulldada symbol">
      <img class="symbol symbol--bottom-right"src="assets/img/cob-symbol.svg" alt="Cult of Bulldada symbol">
      <img class="symbol symbol--bottom-left"src="assets/img/cob-symbol.svg" alt="Cult of Bulldada symbol">
    </div>
    <div class="emblem fade-in fade-in__emblem">
      <img class="emblem__item" src="assets/img/emblem-2.svg">
    </div>
  <main class="main-content fade-in fade-in__main">
    <div class="relative-container">
      <nav class="main-navigation">
        <ul class="main-navigation__list">
          <li><a class="link" href="about.php">About</a></li>
          <li><a class="link" href="god.php">Our God</a></li>
          <li><a class="link" href="mission.php">Our Mission</a></li>
          <li><a class="link" href="bulldada.php">Bulldada</a></li>
          <li><a class="link" href="join.php">Join us</a></li>
        </ul>
      </nav>
      <img class="logo "src="assets/img/logo-final.png" alt="Logotype of Cult of Bulldada Kazakhstan">
    </div>
    <?php // TODO: Remove comments around audio when going live ?>
    <!-- <audio src="assets/sound/background.wav" autoplay></audio> -->
  </main>
<?php @include 'footer.php'; ?>
